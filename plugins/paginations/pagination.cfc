component name="pagination" accessors="true"
{
	...
	public function parseURLparams(required urlParams)
	{
		...
		{
			if(IsSimpleValue(ARGUMENTS.urlParams[local.param]) && LEN(TRIM(ARGUMENTS.urlParams[local.param])) AND ListFindNocase(local.invalidParams, TRIM(local.param)) EQ 0)
			{
	...
	)
	{
		...
		if(StructKeyExists(VARIABLES.rendering, "getLink"))
		{
			VARIABLES.rendering.getLink(
				styles = ARGUMENTS.styles,
				parsedUrlParams = local.parsedUrlParams,
				extra = ARGUMENTS.extra,
				linkText = ARGUMENTS.linkText,
				vc = VARIABLES.serviceArguments.vc
			);
			return "";
		}
	...
	public function renderPagination(vc)
	{
		if(VARIABLES.endPage GT 1)
		{
			...
			VARIABLES.serviceArguments.vc = {};

			if(StructKeyExists(ARGUMENTS, "vc"))
			{
				VARIABLES.serviceArguments.vc = ARGUMENTS.vc;
			}
	...
	public function renderRecordPerPage(intervals = APPLICATION.controller.getSetting("PAGINATION_DEFAULT_PAGE_INTERVALS", "10,25,50,100"))
	{
		...
		local.intervalLinks = [];
		local.recordPerPage = getLink(VARIABLES.pageSize);
		...
		VARIABLES.rendering.renderRecordPerPage(
			intervalLinks = local.intervalLinks,
