<cfcomponent name="forms" hint="Common forms plugin" output="false">
	...
	<cffunction name="init" access="public" output="false">
		...
		<cfset variables.formComponentLocation = variables.alyx.getSetting("customFormComponentLocation", "/alyx.plugins.forms.form")/>
	...
	<cffunction name="initValidationTypes" access="private" output="no">
		...
		<cfset local.customLocations = variables.alyx.getSetting("customFormValidationTypeLocations")/>
	...
	<cffunction name="registerValidationType" output="no">
		...
		<cfset variables.validationTypes[arguments.name] = CreateObject("component", arguments.path & "." & arguments.name).init(alyx = variables.alyx)/>
	</cffunction>
	...
	<cffunction name="initValidationDependencies" access="private" output="no">
		...
		<cfset local.customLocations =  variables.alyx.getSetting("customFormValidationDependencyLocations")/>
		<cfif IsArray(local.customLocations)>
			<cfloop array="#local.customLocations#" index="local.location">
	...
	<cffunction name="InitDisplayTypes" access="private" output="no">
		...
		<cfset local.customLocations =  variables.alyx.getSetting("customFormFieldDisplayTypeLocations")/>
		<cfif IsArray(local.customLocations)>
			<cfloop array="#local.customLocations#" index="local.location">
	...
	<cffunction name="getClientSideValidationScript" output="no">
		...
		<cfloop collection="#local.rc.forms#" item="local.formName">
			...
			<cfif local.form.getShowClientSideValidationScript()>
				...
				<cfset local.context.output.append("function getErrorHeading(){return '" & JSStringFormat(HtmlEditFormat( variables.alyx.getPlugin("snippets").getSnippet("errorheading", "errormessages"))) & "'};")/>
			</cfif>
		</cfloop>
	...
	<cffunction name="getDataSetDataByType" access="private" output="no">
		...
		<cfset var local = StructNew()/>
		<cfset local.data = ""/>
		<cfset local.snippetsPlugin =  variables.alyx.getPlugin("snippets")/>
